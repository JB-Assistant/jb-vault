...
**Source:** Peter Pang (@intuitiveml) — 🚀 Co-founder, CreaoAI. Previously GenAI @ Meta (LLaMA).  xApple. Join the journey ↓
**Date:** 2026-04-28
**Link:** https://x.com/intuitiveml/status/2048912026018484317
**Stats:** 38,614 views · 439 likes · 995 bookmarks · 51 retweets
...
1. **The Grader.** A tri-judge panel that scores every live agent response *(Replaces human QA review and offline benchmark eval)*

1. **The Engineering Pipeline.** Six daily jobs that turn low scores into Linear tickets, draft PRs, and verified fixes *(Replaces manual bug triage, sprint planning, and regression testing)*

1. **The Bridge.** AI-gated grey rollouts where the Grader's scores decide whether new code ships *(Replaces staging environments and release approvals)*
...
Peter Pang, co-founder of CreaoAI (ex-Meta LLaMA team), describes how his team replaced traditional QA with an autonomous "self-healing Agent Harness" — a closed-loop system that grades every AI agent response in production, automatically triages failures into engineering tickets, drafts PRs, verifies fixes, and gates deployments. The system has three components: (1) **The Grader** — a tri-judge panel of three LLM judges that independently score every live agent response on correctness, safety, and tone, using majority vote and a categorical router to assign specialized rubrics; (2) **The Engineering Pipeline** — six daily automated jobs that cluster low-scoring interactions, create Linear tickets, draft fixes, verify them against telemetry, re-grade, and produce nightly reports; and (3) **The Bridge** — AI-gated grey rollouts that route 10% of traffic to new variants and promote/abort based on statistical score comparisons (p < 0.05 over 200+ interactions).

The result: they ship to production 3–8 times per day with no QA team, no staging environment, and no manual release approvals. The grader's scores directly control what ships.

---

## Key Takeaways

1. **Evaluation and QA are the same loop** for AI-native products — a bad agent response is both a metric to chart and a bug to fix. Don't separate them into different teams.

2. **Grade outcomes, not trajectories** — agents often take non-linear, seemingly inefficient paths that still produce correct results. Penalizing the path is counterproductive.

3. **Tri-judge panel with majority vote** eliminates single-model bias — three different LLMs independently score each response, and the median score wins. This catches cases where one model is lenient or harsh.

4. **Categorical routing is critical** — a fast classifier routes each interaction to a domain-specific rubric (e.g., scheduling vs. billing vs. troubleshooting) so the grader uses the right criteria.

5. **Scores without tickets are useless dashboards** — the grader must feed directly into an engineering pipeline that creates tickets, drafts PRs, and verifies fixes. Build both or build neither.

6. **AI-gated grey rollouts replace staging** — route 10% of real traffic to new variants, score head-to-head against baseline, and auto-promote or abort based on statistical significance.

7. **Don't chase "scientific correctness"** — for startups, a good-enough signal that triggers a fix today beats a perfectly defensible benchmark that ships next quarter.

8. **Self-healing = grade → triage → fix → verify → gate** — this is the full loop. Every component runs on the model's own output. No humans in the loop for routine failures.

9. **The competitive advantage** goes to teams that fuse evaluation and QA into a single harness, not teams that bolt Copilot onto traditional workflows.

---

## Hermes-Related Interest

🔥 **Highly relevant to Hermes and all of Junaid's agent-based products.**

- **Direct parallel to Hermes's architecture**: The tri-judge grader concept maps directly to how multi-agent systems can self-evaluate. Hermes already uses agents for task execution — adding an automated grading layer on agent outputs (especially for OttoManagerPro's SMS responses and TireManagerPro's customer interactions) could catch quality issues before customers notice.

- **The "Agent Harness" pattern is applicable to FieldAgent AI**: With 6 parallel agents, FieldAgent already has the complexity where automated grading + triage would pay off. Each agent's output could be scored by a lightweight judge, with failures auto-escalated.

- **Grey rollouts for prompt/model changes**: When swapping Claude models or updating system prompts across the SaaS products, routing 10% of traffic through the new config and scoring it against baseline would de-risk changes significantly.

- **The "score → ticket → PR → verify" pipeline** is exactly the kind of automation loop Junaid values — revenue-protecting, not just informational. Bad agent responses = churn risk. Catching them automatically = retention.

- **Practical for solo dev**: Even without a team, the automated pipeline (grader → ticket → alert) means one person can maintain quality across multiple products without manually reviewing transcripts.
