...
source: "https://x.com/_avichawla/status/2046500537584218438?s=46&t=ub4QNKLqvh3yU44VgGVYlQ"
author:
  - "[[@_avichawla]]"
published: 2026-04-21
created: 2026-04-21
description: "A full breakdown of how one open-source tool cuts your Claude Code session costs by 3x, without any changes to CLAUDE.md, prompts, or models..."
tags:
  - "clippings"
---
![Image](https://pbs.twimg.com/media/HGahAIPaUAAmcUJ?format=jpg&name=large)
...
![Image](https://pbs.twimg.com/media/HGV9cpMa4AAj3Nu?format=jpg&name=large)
...
When CC needs to set up Google OAuth through Supabase, it invokes the search\_docs MCP tool.
...
![Image](https://pbs.twimg.com/media/HGV9gXqaIAAwy6O?format=jpg&name=large)
...
This happens on every search\_docs call, like database queries, storage configuration, and edge function deployment.
...
Supabase’s MCP server does expose some state through individual tools like list\_tables and execute\_sql, but there’s no way to ask “what does my entire backend look like right now?” and get one structured response.

![Image](https://pbs.twimg.com/media/HGV9jIKaYAAyXYq?format=jpg&name=large)
...
To see what this looks like in practice, [InsForge](https://github.com/InsForge/InsForge) (open source with 8k stars) implements this approach.

![Image](https://pbs.twimg.com/media/HGV9mY6boAAG4e4?format=jpg&name=large)
...
![Image](https://pbs.twimg.com/media/HGV9ulhbcAApr-l?format=jpg&name=large)
...
InsForge’s MCP server exposes a lightweight get\_backend\_metadata tool that returns a structured JSON with the full backend topology in a single call:
...
<video preload="none" tabindex="-1" playsinline="" aria-label="Embedded video" poster="https://pbs.twimg.com/amplify_video_thumb/2046180602358247424/img/Fhf6eHR7cZxOOmfu.jpg" style="width: 100%; height: 100%; position: absolute; background-color: black; top: 0%; left: 0%; transform: rotate(0deg) scale(1.005);"><source type="video/mp4" src="blob:https://x.com/eeed2944-6c91-4925-9df4-6894ed70ebc7"></video>

![](https://pbs.twimg.com/amplify_video_thumb/2046180602358247424/img/Fhf6eHR7cZxOOmfu.jpg?name=large)
...
![Image](https://pbs.twimg.com/media/HGV-g0YacAAYoEj?format=jpg&name=large)
...
The agent loaded the supabase skill, discovered the backend state via MCP tools (list\_tables, list\_extensions, execute\_sql), scaffolded the Next.js project, created the database schema, wrote two edge functions (ingest-document and query-document), and deployed everything. The build passed.

![Image](https://pbs.twimg.com/media/HGV-j9TbAAAUn_g?format=png&name=large)

## First problem: login didn’t work

![Image](https://pbs.twimg.com/media/HGV-mEyaUAAOWvl?format=png&name=large)
...
![Image](https://pbs.twimg.com/media/HGV-orYacAAG-m_?format=png&name=large)
...
![Image](https://pbs.twimg.com/media/HGV-rMGawAAXayX?format=jpg&name=large)
...
![Image](https://pbs.twimg.com/media/HGV-tfTakAArSqZ?format=jpg&name=large)
...
After 8 failed attempts, the agent finally figured out what was going on: “The 401s may be happening at the platform’s verify\_jwt gate before our code even runs.”
...
![Image](https://pbs.twimg.com/media/HGV-wMzasAA7Xso?format=jpg&name=large)
...
![Image](https://pbs.twimg.com/media/HGV-ywCagAA35GF?format=jpg&name=large)

This gave the agent a complete picture of what it was working with before it wrote any code.

In the Supabase session, the agent needed multiple MCP calls (list\_tables, list\_extensions, execute\_sql) to piece together a similar understanding, and even then, it missed critical details like the verify\_jwt behavior.

The schema setup ran through 6 CLI commands, all of which succeeded.

![Image](https://pbs.twimg.com/media/HGV-1gVbQAAqtXP?format=jpg&name=large)

The agent enabled pgvector, created the documents and chunks tables (with a vector(1536) column), enabled Row Level Security on both, created the access policies, and set up the match\_chunks similarity search function.
...
![Image](https://pbs.twimg.com/media/HGV-3xsbgAANN-P?format=jpg&name=large)
...
![Image](https://pbs.twimg.com/media/HGV-6a8bAAAJgc9?format=png&name=large)
...
None of this was the agent’s fault. The Supabase platform’s verify\_jwt gate was rejecting the token before the function code ran, and the logs didn’t distinguish between platform-level and code-level rejections.
...
![Image](https://pbs.twimg.com/media/HGV_CS3bEAERQP1?format=jpg&name=large)
...
The code, the skills, and the CLI are all on its GitHub repo: [https://github.com/InsForge/InsForge](https://github.com/InsForge/InsForge)
