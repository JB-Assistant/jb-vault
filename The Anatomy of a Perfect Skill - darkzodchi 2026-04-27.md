...
**Source:** darkzodchi (X: @zodchiii)  
**Date:** 2026-04-27  
**Link:** https://x.com/zodchiii/status/2048345453096313005
...
Counterintuitive but powerful. The best skills explicitly list what's out of scope. From Anthropic's official PDF skill:
...
Why this matters: when a user asks for something the skill can't do, Claude doesn't try and fail. It either picks a different skill or asks for clarification. You get fewer broken outputs and more correct routing.

This pattern shows up in 70% of high-quality skills and almost never in low-quality ones.

### Pattern 6: Keep it under 500 lines

Every skill loads into Claude's context when invoked. A 2000-line skill eats 5000+ tokens before doing anything. The longer the skill, the more chance Claude loses focus halfway through and starts ignoring instructions at the bottom.
...
Good skills compound in the opposite direction. Each well-written skill makes Claude better at picking the right one for any task. The skills become a system, not a collection.

The skills folder of someone who's been doing this for 6 months looks completely different from someone who just started. Same Claude, completely different output quality.

---

## Summary

This article by @zodchiii reverse-engineers 100 high-performing Claude Code skills and distills the 6 patterns that separate effective skills from dead ones. The core insight: most skills fail not because the instructions are bad, but because Claude doesn't know WHEN to invoke them, the output format is unspecified, or they're bloated beyond usefulness.

---

## Key Takeaways

1. **Description is the #1 lever** — include WHEN to trigger, not just WHAT the skill does. Front-load use cases in the first 250 chars. Under 50 chars = 3-5x less invocation.
2. **Be directive, not conversational** — imperative verbs, numbered steps, explicit output formats. No polite questions.
3. **Specify output format explicitly** — without it, Claude produces inconsistent results every time.
4. **Include a "read first" step** — tell Claude to inspect existing code/files before generating. This is what makes output match your project.
5. **Define what the skill does NOT do** — out-of-scope sections prevent broken outputs and improve routing to the right skill.
6. **Keep under 500 lines** — long skills eat tokens and Claude loses focus. Use progressive disclosure (split into supporting files).
7. **Checklist for fixing broken skills:** 3+ trigger keywords, imperative verbs, explicit format, read-first step, out-of-scope section, under 500 lines.

---

## Hermes-Related Interest

Directly applicable to Hermes skills system. The 6 patterns map 1:1 to how Hermes loads and routes skills via `skill_view()`. Key actionable insights:
- Audit existing Hermes skills for vague descriptions (under 50 chars = dead skill)
