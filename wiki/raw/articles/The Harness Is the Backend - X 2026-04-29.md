...
**Date:** 2026-04-29  
**Original date:** Tue Apr 28 14:49:59 +0000 2026  
**Link:** https://x.com/mfpiccolo/status/2049139067359568032  
**Stats at capture:** 175,201 views, 595 likes, 1,777 bookmarks, 75 reposts
...
Mike Piccolo argues that the important AI infrastructure question is not model choice, but how much backend-like infrastructure is needed around the model. He frames today’s agent harness debate as a spectrum: Anthropic uses a thin harness that trusts the model, OpenAI adds more orchestration, CrewAI mixes deterministic flows with autonomous agents, and LangGraph encodes the most workflow logic.

His core claim is that the harness should not live as a separate layer beside the backend. Instead, agent tools, memory, orchestration, queues, state, schedules, observability, and services should collapse into shared backend primitives: workers, triggers, and functions. In that model, an agent is just another worker, its tools are functions, its memory is state, and its orchestration is triggers/composition.

---

## Key Takeaways

1. The model is not the product; the infrastructure/harness around it is what makes it useful.
2. Harness design sits on a spectrum: thin model-trusting loops vs thick deterministic workflow encoding.
3. Current agent stacks often split harness and backend, creating duplicated retries, timeouts, queues, and logs.
4. Debugging agentic systems gets messy fast because stochastic agent paths multiply across backend services.
5. Piccolo’s backend abstraction: workers orchestrate work, triggers invoke work, functions do the work.
6. If agents are workers, their tools are functions, their memory is state, and their orchestration is triggers.
7. Live discovery matters: agents should see the current function/tool catalog, not stale hardcoded context.
8. Live observability matters: one trace should span agent tool call → queue → downstream function → state write.
9. Agent-created sandbox workers make infrastructure recursive: agents can safely create new isolated capabilities at runtime.
10. The category collapse is the big idea: harness/backend/cloud/edge/human-written/agent-created services become the same primitive.

---

## Hermes-Related Interest

Very relevant to Hermes and Junaid’s SaaS architecture. It pushes the agent-harness idea one step further: Hermes should not merely call a backend; Hermes-style agents can become backend participants with common primitives for tools, state, schedules, queues, permissions, and traces. For TireManagerPro/OttoManagerPro, this suggests a strong architecture/product angle: shop agents as workers. Missed-call recovery, quote follow-up, appointment reminders, inventory checks, and manager briefings all become functions triggered by events/state/cron, with the agent participating in the same observable workflow system as the app.
